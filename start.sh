...
#   ./start.sh                  # start (padrão): pull + build + sobe + health
#   ./start.sh restart          # stop + git pull + rebuild + start (exige árvore git limpa)
#   ./start.sh restart-local    # stop + start rápido, SEM pull e SEM rebuild
#
# O que o start (padrão) faz, em ordem:
...
# ─── guarda de árvore limpa (restart) ────────────────────────
# Evita que o git pull do restart atropele trabalho em andamento.
ensure_git_clean() {
    if [ -n "$(git status --porcelain)" ]; then
        echo -e "${RED}⚠ Há mudanças locais não commitadas. Faça commit (ou stash) antes do restart,${NC}"
        echo -e "${RED}  ou use './start.sh restart-local' para reiniciar sem git pull.${NC}"
        exit 1
    fi
}
...
# ─── git pull ────────────────────────────────────────────────
do_git_pull() {
    echo -e "${BOLD}»${NC} Buscando atualizações do GitHub..."
    if [ -d .git ]; then
...
show_logs() {
    echo ""
    echo -e "${BOLD}»${NC} Aguardando container ficar pronto..."
    sleep 3
...
health_check() {
    echo ""
    echo -e "${BOLD}»${NC} Testando /health..."
...
stop)
    banner
    echo -e "${BOLD}»${NC} Parando containers..."
    docker compose stop
    echo -e "${GREEN}✅ Containers parados${NC}"
    ;;
restart)
    # stop + git pull + rebuild + start (exige árvore git limpa).
    banner
    check_env
    ensure_git_clean
    do_git_pull
    ...
    ;;
restart-local)
    # Reinício rápido (válvula de escape): SEM git pull e SEM rebuild.
    banner
    check_env
    echo -e "${BOLD}»${NC} Parando containers..."
    docker compose stop
    echo -e "${BOLD}»${NC} Subindo de novo (sem pull, sem rebuild)..."
    docker compose up -d
    show_logs
    health_check
    echo -e "${GREEN}✓ restart-local concluído${NC}"
    ;;
status)
    docker compose ps
    ;;
logs)
    ...
    ;;
*)
    echo "Uso: $0 {start|stop|restart|restart-local|status|logs}"
    echo ""
    echo "  start          → pull + build + sobe + health (padrão, sem argumento)"
    echo "  stop           → para os containers"
    echo "  restart        → stop + git pull + rebuild + start (exige árvore git limpa)"
    echo "  restart-local  → stop + start rápido, SEM pull e SEM rebuild"
